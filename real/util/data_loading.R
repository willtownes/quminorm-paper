...
parse_sra_csv<-function(bp,kallisto_tech,species_map,kallisto_threads=8){
  ...
  fq1<-fp("data/original/fastq",paste0(srr,"_1.fastq.gz"))
  fq2<-fp("data/original/fastq",paste0(srr,"_2.fastq.gz"))
...
bus2genecount_all<-function(bp="./"){
  ...
  for(i in 1:nrow(coldata)){
    ...
    if(!file.exists(ofile)){
      res<-ec2counts(pth,coldata$ScientificName[i],ensembl_version=95,bc_wl=bc_wl)
